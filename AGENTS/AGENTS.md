...
1. Смотри файлы в этой же директории.
2. **Две первичные абстракции Eve:** **`$eve_surface`** (поверхность: палитра, вариант, состояния) и **`text`** в смысле модуля/FQN **`$eve_text_*`** — всё, что в референсе [design-hypothesis](../../../design-hypothesis/README.md) отнесено к **Typography**. Слово *Text* короче *Typography*, меньше символов в именах. Не используем `mol_view` как базу экранов.
3. Остальные компоненты опираются на эти два слоя там, где нужны и «материал», и типографика. Прямое использование `mol_*` минимизируем; наследуемся и перебиваем токены/тему там, где нужно. **Целевой паритет набора** — с [Ant Design](../../../ant-design/components) в `references/ant-design` (см. [TODO_COMPONENTS.md](../TODO_COMPONENTS.md)); `@mo/ui` и [design-hypothesis](../../../design-hypothesis/README.md) остаются вспомогательными референсами.
4. **Темизация `@mo/ui` (MO):** провайдеры саббренда, `SubbrandWrapper`, трёхслойные токены — это **не целевая модель для Eve**, а специфика пакета MO и компромисс из [ADR 0001](../../../../decisions/0001-colors/README.md) («лишняя абстракция» ради Figma и CSS). В Eve ориентируемся на упрощение, ближе к `$mol_theme` и коротким пресетам (задачи в [TODO.md](../TODO.md)).
5. Тесты: см. [testing.md](testing.md) (`$mol_test`, `*.test.ts`, `/-/test.html` и прогон `node.test.js`).
